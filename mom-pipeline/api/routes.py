...
async def process_media(file: UploadFile = File(...)):
    ...
    logger.info(f"API Request received: Upload file '{safe_filename}'")
    ...
    try:
        ...
        logger.error(f"API processing error on '{safe_filename}': {str(e)}")
        raise HTTPException(
...
async def export_pdf(filename: str):
    ...
    json_path = settings.OUTPUT_DIR / f"{safe_name}_mom.json"
    pdf_path = settings.OUTPUT_DIR / f"{safe_name}_mom.pdf"

    if not json_path.exists():
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"No processed data found for '{safe_name}'. Process the file first."
        )
    ...
    try:
        ...
        return FileResponse(
            path=str(pdf_path),
            filename=f"{safe_name}_MoM.pdf",
            media_type="application/pdf"
        )
    except Exception as e:
        logger.error(f"Failed to generate PDF for '{safe_name}': {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def export_docx(filename: str):
    ...
    safe_name = Path(filename).stem
    json_path = settings.OUTPUT_DIR / f"{safe_name}_mom.json"
    docx_path = settings.OUTPUT_DIR / f"{safe_name}_mom.docx"
    ...
    try:
        ...
        return FileResponse(
            path=str(docx_path),
            filename=f"{safe_name}_MoM.docx",
            media_type="application/vnd.openxmlformats-officedocument.wordprocessingml.document"
        )
    except Exception as e:
        logger.error(f"Failed to generate DOCX for '{safe_name}': {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
